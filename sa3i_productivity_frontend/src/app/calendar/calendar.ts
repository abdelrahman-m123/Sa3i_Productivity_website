...
export class Calendar {
  ...
  dropTaskOnDay(event: CdkDragDrop<Task[]>, isoDate: string): void {
    ...
    if (!task || this.toDateKey(task.dueDate) === isoDate) {
      return;
    }
    ...
    this.taskService.updateTask(task._id, { dueDate: isoDate }).subscribe({
      next: () => {
        this.ref.detectChanges();
      },
      error: () => {
  ...
  private buildCalendar(): void {
    ...
    this.unscheduledTasks = this.tasks.filter((task) => !this.toDateKey(task.dueDate));
